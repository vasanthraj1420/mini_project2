...
if web=="📉Data Analysis & Visualization":
    ...
    if s=="Sector-wise Performance":
        ...
        conn = pymysql.connect(**db_config)
        query = '''SELECT sector, Yearly_Return FROM Sector_wise_Performance'''
        sector_performance = pd.read_sql(query, conn)
        conn.close()
        st.write("Sector Performance Data", sector_performance)
        plt.figure(figsize=(10, 6))
        plt.bar(sector_performance['sector'], sector_performance['Yearly_Return'], color='skyblue')
    ...
    if s=="Stock Price Correlation":
        ...
        plt.title("Sector-Level Correlation Heatmap (Positive and Negative)")
        plt.xlabel("Sectors")
        plt.ylabel("Sectors")
        plt.show()
        st.pyplot(plt)
